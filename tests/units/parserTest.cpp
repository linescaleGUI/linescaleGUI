...
TEST_F(ParserTest, ParseToShortPackage) {
    QByteArray toShortPackage("-00.01N000.00?NF41\r"); //missing 'R' at the beginning -> length = 19
    bool dataOk = parser.parsePackage(toShortPackage, result);
    ASSERT_FALSE(dataOk);
}

TEST_F(ParserTest, ParseToLongPackage) {
    QByteArray toLongPackage("RR-00.01N000.00?NF41\r"); //one 'R' to many at the beginning -> length = 21
